...
/* dbo.spGDConsultarConfiguracionMigracionArchivosIndices 25 */
CREATE PROCEDURE [dbo].[spGDConsultarConfiguracionMigracionArchivosIndicess] 
...
Declare @funcionRutaFisico Nvarchar(250)
Declare @funcionRutaIndice Nvarchar(250)
Declare @granQuery Nvarchar(2000)
--Set @declares= '@ruta nvarchar(200) output,@rutaIndice nvarchar(200) output'

Set @funcionRutaFisico = ' ' + (case when (@fncn_clclo_drccn is Null) then '@ruta' else  (@fncn_clclo_drccn + '('+ Cast(@cnsctvo_cdgo_cnfgrcn As nvarchar(5)) + ') + ''\''') End) + ' as drccn_archvo_fsco'
Set @funcionRutaIndice =  (case when (@fncn_clclo_indce is Null) then '@rutaIndice' else  (@fncn_clclo_indce +'('+ Cast(@cnsctvo_cdgo_cnfgrcn As nvarchar(5)) + ') + ''\''') End) + ' as drccn_archvo_indce'
...
Where	cnsctvo_cdgo_cnfgrcn	= @cnsctvo_cdgo_cnfgrcn '
print @granQuery
Exec sp_executesql @granQuery,@declares,@ruta ,@rutaIndice ,@cnsctvo_imgn_incl,@cnsctvo_cdgo_cnfgrcn
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spGDConsultarConfiguracionMigracionArchivosIndicess] TO [GestionDocumental]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spGDConsultarConfiguracionMigracionArchivosIndicess] TO [websos]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spGDConsultarConfiguracionMigracionArchivosIndicess] TO [visosservice_role]
    AS [dbo];
