...
/*---------------------------------------------------------------------------------
* Metodo o PRG:			spVUConsultarSoportesAfiliacionJoin
* Desarrollado por:		<\A	Ing. Jorge Marcos Rincon Ardila	A\>
* Descripcion:			<\D	Consultar Soportes de Afiliacion para el Visor Unificado	D\>
* Observaciones:		<\O		O\>
* Parametros:			<\P		P\>
* Fecha Creacion:		<\FC	13/03/2008	FC\>
*---------------------------------------------------------------------------------
...
*---------------------------------------------------------------------------------*/
--exec spVUConsultarSoportesAfiliacionJoin null, null, null, null, null, null, null, null, '2009/01/22', '2009/01/24', null 
CREATE PROCEDURE [dbo].[spVUConsultarSoportesAfiliacionJoin] 
...
BEGIN
	...
	Declare @cnsctvo_cdgo_tpo_dcmnto		int
	
	If @Tipo_Documento Is Not Null
		Begin
			Set @cnsctvo_cdgo_tpo_dcmnto = Isnull((Select cnsctvo_cdgo_tpo_dcmnto from dbo.tbTiposDocumento where dscrpcn_tpo_dcmnto = @Tipo_Documento), null)
		End
	...
	Begin Transaction
		...
		Insert Into @tmpSoportes(
			...
			0,							DatePart(Year,a.fcha_crga),	null, 
			null,				null,						null, 
			convert(int, a.indce1),		a.indce3,					null, 
			null,						a.nmro_blsa,				null, 
			null,						null,						'Novedad', 
			null,						null, 
			'\ImagenesOUT\Digitalizacion\Afiliacion\' 
			...
			+ a.nmbre_archvo, 
			'Sinva',					a.fcha_dgtlzcn,				null 
		From	dbo.tbRegistrosArchivosIndices a 
		Where	(a.cnsctvo_cdgo_tpo_dcmnto = 8) 
		And		(@Numero_Id Is Null Or a.indce3 = @Numero_Id) 
		And		(@Numero_Formulario Is Null Or a.indce1 = @Numero_Formulario) 
		And		(@Bolsa Is Null Or a.nmro_blsa = @Bolsa) 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [GestionDocumental]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [Administrador Gestion Documental]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [Consultor Gestion Documental]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [Gestion Documental Web]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [Liquidador Recobros]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [Coordinador Parametros Recobro]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [websos]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[spVUConsultarSoportesAfiliacionJoin] TO [visosservice_role]
    AS [dbo];
